...
## 6. ตั้งเครื่องหน้าร้าน (iPad / iPhone) — เพิ่มลงหน้าจอโฮม
ทำครั้งเดียวต่อเครื่อง แล้วเปิดใช้งานจากไอคอนแทนการเปิดใน Safari

1. เปิดเว็บใน **Safari** (ผ่าน Chrome บน iOS ไม่ได้ ต้อง Safari เท่านั้น)
2. ปุ่มแชร์ (⬆️) → **เพิ่มลงหน้าจอโฮม** → เพิ่ม
3. เปิดจากไอคอน ☕ บนหน้าจอโฮม

ได้อะไร: เปิดมา **เต็มจอ ไม่มีแถบ URL** → คำว่า "ไม่ปลอดภัย" ไม่โผล่ และหายอาการ
**แตะปุ่มแล้วไม่ตรง ต้องกดต่ำกว่าปุ่ม** (เกิดจากแถบ URL หด/กางเอง แล้วพื้นที่รับการแตะเลื่อนไม่ทันภาพ)

- ไม่ตั้ง `start_url` ใน `frontend/public/manifest.json` — ตั้งใจ เพื่อให้แต่ละเครื่องเปิดมาที่หน้าที่ตัวเองปักไว้
  (เครื่องหน้าร้านปักหน้าสั่งสินค้า `/` · เครื่องแอดมินปัก `/admin/orders` ได้)
- แอปที่เปิดจากไอคอนมี storage แยกจาก Safari → **admin ต้อง login ใหม่หนึ่งครั้ง** ในแอป
- ลูกค้าที่สแกน QR เข้ามาสั่งยังเห็นแถบ URL ตามปกติ (แก้ได้ด้วย HTTPS เท่านั้น — ดูท้ายไฟล์)
- **Android** ต้องมี HTTPS ก่อน Chrome ถึงจะติดตั้งเป็นแอปได้ ตอนนี้จะได้แค่ทางลัดที่เปิดในแท็บปกติ
